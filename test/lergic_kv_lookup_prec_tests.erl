-module(lergic_kv_lookup_prec_tests).
...
rel(boring_number,[V],Val,top) ->
	[{[VV],ValV} || N <- [0],
	       VV <- lergic:bind(V,N),
			   ValV <- lergic:bind(Val,true)];
rel(cool_number,[V],Val,top) ->
	[{[VV],ValV} || N <- [2,3,4],
	       VV <- lergic:bind(V,N),
			   ValV <- lergic:bind(Val,true)];
rel(cool_number,[V],Val,extra_cool) ->
	[{[VV],ValV} || N <- [5,6,7],
	       VV <- lergic:bind(V,N),
			   ValV <- lergic:bind(Val,true)];
...
rel(link_between,[L1,L2],LinkPair,top) ->
